    ...
    return {
      ...
      requestPasswordReset: (email: string, ip: string) =>
        Effect.gen(function* () {
          ...
          yield* emailService
            .sendPasswordResetEmail(canonicalEmail, rawToken)
            .pipe(Effect.catchAll((error) => Effect.logError(`Failed to send email: ${error}`)));
      ...
      resetPassword: (token: string, newPassword: string) =>
        Effect.gen(function* () {
          ...
          if (updatedUser.passwordChangedAt) {
            ...
            yield* userAuthCache
              .setPasswordChangedAt(updatedUser.id, timestamp)
              .pipe(Effect.catchAll((error) => Effect.logWarning(`Failed to update cache: ${error}`)));
          }

          yield* Effect.logInfo(`Password reset successful for user ${tokenRecord.userId}`);
